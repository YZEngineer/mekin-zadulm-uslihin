...
class CategoryLessonsScreen extends StatefulWidget {
  ...
  const CategoryLessonsScreen({super.key, required this.categoryName});
...
class _CategoryLessonsScreenState extends State<CategoryLessonsScreen> {
  ...
  Future<void> _loadLessons() async {
    ...
    try {
      final lessons = await lesson_dao.getLessonByCategory(widget.categoryName);
      setState(() {
        _lessons = lessons;
        _isLoading = false;
      });
  ...
  Future<void> _deleteLesson(Lesson lesson) async {
    ...
    if (confirmed == true) {
      try {
        await lesson_dao.delete(lesson.id!);
        await _loadLessons();
        if (mounted) {
