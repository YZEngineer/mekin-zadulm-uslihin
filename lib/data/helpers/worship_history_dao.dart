...
class WorshipHistoryDao {
  ...
  Future<int> update(WorshipHistory history) async {
    if (history.id == null) {throw ArgumentError('لا يمكن تحديث سجل عبادة بدون معرف');}
    return await _databaseHelper.update(_tableName, history.toMap(), 'id = ?', [history.id]);}
  ...
  Future<List<WorshipHistory>> getAll() async {
    final result = await _databaseHelper.query(_tableName, orderBy: 'date DESC');
  ...
  Future<List<WorshipHistory>> getByDate(DateTime date) async {
    ...
    final result = await _databaseHelper.query(
      _tableName, where: 'date = ?', whereArgs: [formattedDate], orderBy: 'id DESC');
  ...
  Future<List<WorshipHistory>> getByType(String type) async {
    final result = await _databaseHelper.query(
      _tableName, where: 'type = ?', whereArgs: [type], orderBy: 'date DESC');
  ...
  Future<List<WorshipHistory>> getByDateRange(
    ...
    final result = await _databaseHelper.query(_tableName, where: 'date BETWEEN ? AND ?',
     whereArgs: [formattedStartDate, formattedEndDate], orderBy: 'date DESC');
    return result.map((map) => WorshipHistory.fromMap(map)).toList();
  }
  ...
  Future<List<WorshipHistory>> search(String keyword) async {
    final result = await _databaseHelper.query(_tableName, where: 'type LIKE ? OR details LIKE ?',
     whereArgs: ['%$keyword%', '%$keyword%'], orderBy: 'date DESC');
    return result.map((map) => WorshipHistory.fromMap(map)).toList();}

  /// الحصول على أنواع العبادات المتاحة
  Future<List<String>> getAvailableTypes() async {
    final result = await _databaseHelper.rawQuery('SELECT DISTINCT type FROM $_tableName ORDER BY type ASC');
